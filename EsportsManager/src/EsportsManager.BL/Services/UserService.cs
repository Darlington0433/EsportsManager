...
/// <summary>
/// User service implementation - áp dụng Single Responsibility Principle
/// Chỉ lo về business logic liên quan đến User
/// Áp dụng Dependency Inversion Principle - phụ thuộc vào abstraction (IUserRepository)
/// </summary>
public class UserService : IUserService
{
    ...
    public async Task<AuthenticationResult> AuthenticateAsync(LoginDto loginDto)
    {
        try
        {
            ...
            if (user.Status != UserStatus.Active)
            {
                ...
                return AuthenticationResult.Failure("Tài khoản không hoạt động");
            }

            // Verify password
            // For demo, we're using plain text comparison instead of hashing
        ...
        catch (Exception ex)
        {
            ...
            return AuthenticationResult.Failure("Đã xảy ra lỗi trong quá trình xác thực");
        }
    }
    ...
    public async Task<BusinessResult<UserDto>> RegisterAsync(RegisterDto registerDto)
    {
        try
        {
            ...
            {
                return BusinessResult<UserDto>.Failure("Email đã tồn tại");
            }
        ...
        catch (Exception ex)
        {
            ...
            return BusinessResult<UserDto>.Failure("Đã xảy ra lỗi trong quá trình đăng ký");
        }
    }
    ...
    /// Update a user
    /// </summary>
    public async Task<BusinessResult<UserDto>> UpdateUserAsync(UpdateUserDto updateUserDto)
    {
        try
        {
            if (updateUserDto == null)
            {
                return BusinessResult<UserDto>.Failure("Update data cannot be null");
            }

            var existingUser = await _userRepository.GetByIdAsync(updateUserDto.Id);
            if (existingUser == null)
            {
                _logger.LogWarning("Update attempt for non-existent user: {UserId}", updateUserDto.Id);
                return BusinessResult<UserDto>.Failure("User not found");
            }
            ...
            {
                _logger.LogError("Failed to update user: {UserId}", updateUserDto.Id);
                return BusinessResult<UserDto>.Failure("Failed to update user");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating user: {UserId}", updateUserDto.Id);
            return BusinessResult<UserDto>.Failure($"Error updating user: {ex.Message}");
        }
    }
    ...
    /// Deactivate a user
    /// </summary>
    public async Task<BusinessResult<bool>> DeactivateUserAsync(int userId)
    {
        try
        {
            ...
            if (existingUser == null)
            {
                _logger.LogWarning("Deactivate attempt for non-existent user: {UserId}", userId);
            ...
            else
            {
                ...
                return BusinessResult<bool>.Failure("Failed to deactivate user");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error deactivating user: {UserId}", userId);
            return BusinessResult<bool>.Failure($"Error deactivating user: {ex.Message}");
        }
    }
    ...
    /// Activate a user
    /// </summary>
    public async Task<BusinessResult<bool>> ActivateUserAsync(int userId)
    {
        try
        {
            var existingUser = await _userRepository.GetByIdAsync(userId);
            if (existingUser == null)
            {
                _logger.LogWarning("Activate attempt for non-existent user: {UserId}", userId);
                return BusinessResult<bool>.Failure("User not found");
            }
            ...
            else
            {
                ...
                return BusinessResult<bool>.Failure("Failed to activate user");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error activating user: {UserId}", userId);
            return BusinessResult<bool>.Failure($"Error activating user: {ex.Message}");
        }
    }
}
